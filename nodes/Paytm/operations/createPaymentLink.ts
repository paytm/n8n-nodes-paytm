...
import {
	formatPaytmCreateLinkExpiryFromNormalizedIso,
	formatPaytmDdMmYyyyFromYyyyMmDd,
	normalizeDateOnlyParam,
...
/** Paytm expiry: use ISO date/time **digits** as India wall clock (see `formatPaytmCreateLinkExpiryFromNormalizedIso`). */
function formatExpiryDateForBody(value: unknown): string {
	const rawDt = normalizeDateTimeParam(value);
	if (rawDt) {
		const literal = formatPaytmCreateLinkExpiryFromNormalizedIso(rawDt);
		if (literal) return literal;
	}
	const ymd = normalizeDateOnlyParam(value);
	if (ymd) {
		const lit = formatPaytmDdMmYyyyFromYyyyMmDd(ymd);
		if (lit) return lit;
	}
	return '';
}

function parseBoolFromOptionalValue(valStr: string, fieldLabel: string, node: INode, itemIndex: number): boolean {
...
const OPTIONAL_LINK_COLLECTION_OPTIONS_COMMON: INodeProperties[] = [
	...
	},
	{
		...
		type: 'dateTime',
		default: '',
		description: 'Link expiry date ("dd/mm/yyyy hh:mm:ss" or "dd/mm/yyyy")',
	},
	{
	...
	},
	{
		...
		placeholder: '1',
		description:
			'Max number of payments allowed for this link',
	},
	{
	...
	},
	{
		...
		default: false,
		description:
			'Whether to create only one order on this link',
	},
	{
...
export async function executeCreatePaymentLink(
	...
	const linkDescriptionRaw = (this.getNodeParameter('linkDescription', itemIndex) as string)?.trim();
	...
	const linkType = this.getNodeParameter('linkType', itemIndex) as CreatePaymentLinkBody['linkType'];
	...
	if (linkType === 'GENERIC') {
		...
		const fromFixedOnly = this.getNodeParameter('additionalFieldsFixed', itemIndex, {}) as IDataObject;
	...
	body.mid = (await this.getCredentials('paytmApi')).merchantId as string;
